...
export default function DomeLabel({ order }) {
  ...
  const [oswm, setOswm] = useState()
  useEffect(() => {
    if (!order) return;

    const fetchDimensions = async () => {
      try {
        const response = await axios.get(`${apiUrl}/oswm/${order.trackingId}`);
        if (response.data.success) {
          setOswm(response.data)
        }
      } catch (error) {
        ...
        <div className="grid grid-cols-2 gap-2 items-center">
          ...
          <div className="flex flex-col text-sm border-r-[1px] border-gray-300 pr-2">
            ...
            <div className="grid grid-cols-2 text-center">
              <div className="border-r-[1px] border-gray-300">KG {oswm?.data?.kg ?? "---"}</div>
              <div>CM {oswm?.data?.cm ?? '---'}</div>
            </div>
          </div>
